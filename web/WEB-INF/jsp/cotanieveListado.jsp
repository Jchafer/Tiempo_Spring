...
<html>
    ...
    <body>
        ...
        <header>
            <div class="container">
                <h1>Lista de Cotas de Nieve</h1><br>
            </div>
        </header>
        <div class="container">
                ...
                <div class="row-fluid">
                    <div class="span12">
                        <table class="table table-sm">
                            ...
                            <tbody>
                                <%
                                    for (Cotanieve cotanieve : cotanieves) {
                                %>
                                <tr class="table-active">
                                    <td><a href="<%=request.getContextPath()%>/cotanieve/readForUpdate.html?id=<%=cotanieve.getIdCotaNieve()%>&usuPass=<%=usuario.getId()%>" title="Editar" ><%=cotanieve.getIdCotaNieve()%></a></td>
                                    <td><%=HtmlUtils.htmlEscape(cotanieve.getPeriodo())%></td>
                                    <td><a href="<%=request.getContextPath()%>/dia/readForUpdate.html?id=<%=cotanieve.getDia().getIdDia()%>&usuPass=<%=usuario.getId()%>" title="Editar" ><%=cotanieve.getDia().getIdDia()%></a></td>
                                    <td>
                                        <a href="<%=request.getContextPath()%>/cotanieve/readForDelete.html?id=<%=cotanieve.getIdCotaNieve()%>&usuPass=<%=usuario.getId()%>" title="Borrar" ><i class="bi bi-trash"></i></a>
                                    </td>
                                </tr>
                                <%
                                    }
